...
ncfile.close()
nc.close()
[cg2117@login-c src]$ cat coupling_pt2.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
MELT = melt_d
AREA = xr.open_dataset("/rds/general/user/cg2117/home/netcdf/gridcell_area_rahu_v3.nc")['area']
...
ncfile = Dataset(os.path.join("/rds/general/user/cg2117/ephemeral/jules-output/"+RC+"/"+SC+"/"+RC+"_"+SC+"_"+"coupled_jules_oggm_00_99.nc"), mode="w", format="NETCDF4")     # WOULD BE BETTER TO HAVE THIS AS rcp45_ACCESS1-0_coupled_jules_oggm_00_99.nc in a separate folder

#ncfile = Dataset("/home/clara/rahu_data/JULES_OGGM/rcp45_ACCESS1-0_coupled_jules_oggm_00_99.nc", mode="w", format="NETCDF4")
#ncfile = Dataset("/home/clara/rahu_data/JULES_OGGM/historical_00_18/coupled_jules_oggm_historical_00_18.nc", mode="w", format="NETCDF4")
lat_dim = ncfile.createDimension('lat', 75)
lon_dim = ncfile.createDimension('lon', 102)
#time_dim = ncfile.createDimension('time', 6939)
time_dim = ncfile.createDimension('time', 36525)
lat = ncfile.createVariable('lat', np.float64, ('lat',))
lat.units = 'degrees_north'
lat.long_name = 'latitude'
